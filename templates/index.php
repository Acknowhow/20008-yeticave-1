    ...
    <ul class="promo__list">
        <?php foreach ($categories as $category => $value) : ?>
            <li class="promo__item promo__item--<?= htmlspecialchars($category) ?>">
                <a class="promo__link"
                   href="all-lots.html"><?= htmlspecialchars($value) ?></a>
            </li>
        <?php endforeach; ?>
    </ul>
</section>
<section class="lots">
    ...
    <ul class="lots__list"><?php if (empty($lots)) : ?><?= 'На данный момент нет открытых лотов' ?>
        <?php else: ?><?php foreach ($lots as $lot) : ?>
            <li class="lots__item lot">
            <div class="lot__image">
                <img src="<?= htmlspecialchars($lot['lot_img_url']) ?>"
                     width="350"
                     height="260"
            ...
            <div class="lot__info">
                ...
                <h3 class="lot__title">
                    <a class="text-link"
                       href="lot.php?lot_id=<?= $lot['lot_id'] ?>"><?= htmlspecialchars($lot['lot_name']); ?>
                    </a>
                </h3>
                <div class="lot__state">
                    ...
                    <div class="lot__timer timer"><?php convertT ?></div>
                </div>
            </div>
