...
<?php if (empty($lots)) : ?><?= 'На данный момент нет открытых лотов' ?><?php else: ?><?php foreach ($lots as $lot) : ?>
        <li class="lots__item lot">
            <div class="lot__image">
                <img src="<?= htmlspecialchars($lot['lot_img_url']) ?>" width="350" height="260" alt="<?= htmlspecialchars($lot['lot_name']) ?>">
            </div>
            <div class="lot__info">
                <span class="lot__category"><?= $lot['lot_category'] ?></span>
                <h3 class="lot__title">
                    <a class="text-link" href="lot.php?lot_id=<?= $lot['lot_id'] ?>"><?= htmlspecialchars($lot['lot_name']); ?></a>
                </h3>
